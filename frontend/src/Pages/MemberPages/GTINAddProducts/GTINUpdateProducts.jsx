...
const GTINUpdateProducts = () => {
    ...
    useEffect(() => {
        ...
        const fetchProductDetails = async () => {
            try {
                const response = await gs1Request.get(`/products?id=${productId}`);
    ...
    const handleCountryOfSales = async () => {
        try {
            const response = await gs1Request.get('/getAllcountryofsale');
            // console.log(response.data);
            const data = response.data;
    ...
    const handleUnitCode = async () => {
        try {
            const response = await gs1Request.get('/getAllunit');
            // console.log(response.data);
            const data = response?.data;
            setUnitCode(data);
    ...
    const handleProductTypeData = async () => {
        try {
            const response = await gs1Request.get('/productTypes');
            // console.log(response.data);
            const data = response.data;
    ...
    const handleBrandNamesEnglishArabic = async () => {
        try {
            const response = await gs1Request.get(`/brands?user_id=${memberData?.id}`);
            // console.log(response.data);
            const data = response.data;
            const name = data.map((country) => country.name);
    ...
    const handleProductDescriptionLanguages = async () => {
        try {
            const response = await gs1Request.get('/getAllprod_desc_languages');
            // console.log(response.data);
            const data = response.data;
    ...
    const handleAllPackageType = async () => {
        try {
            const response = await gs1Request.get('/getAllproductPackag');
            console.log(response.data);
            const data = response.data;
            const PackageName = data.map((country) => country.name);
            setPackageType(PackageName);
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            const response = await gs1Request.put(
    ...
    const handleAutoCompleteInputChange = async (event, newInputValue, reason) => {
        ...
        try {
            ...
            const res = await gs1Request.get(`/gpc/search?term=${newInputValue}`,
                {
    ...
    const handleHsCodeAutoCompleteInputChange = async (event, newInputValue, reason) => {
        ...
        try {
            ...
            // Create a new AbortController
            abortControllerRef.current = new AbortController();
            const res = await gs1Request.get(`/hsCode/searchHsCodes?term=${newInputValue}`,
                {
                    signal: abortControllerRef.current.signal
                })
                        ...
                        <form onSubmit={handleFormSubmit}>
                            ...
                            <div className="">
                                ...
                                <div className="popup-footer">
                                    ...
                                    <button type='submit' className="bg-green-500 hover:bg-primary text-white py-2 px-3 rounded-sm" id="gtin-form"> {t('Update Changes')}</button>
                                </div>
                            </div>
                        </form>
